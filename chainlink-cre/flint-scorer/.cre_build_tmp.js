...
var PAGE_SIZE = 10;
var MAX_PAGES = 1;
function githubGet(runtime2, token, path) {
...
function githubGetPaginated(runtime2, token, basePath) {
  const all = [];
  const separator = basePath.includes("?") ? "&" : "?";
  for (let page = 1;page <= MAX_PAGES; page++) {
    const path = `${basePath}${separator}per_page=${PAGE_SIZE}&page=${page}`;
    const results = githubGet(runtime2, token, path);
...
function fetchMergedPRs(runtime2, token, owner, repo, since) {
  const raw = githubGetPaginated(runtime2, token, `/repos/${owner}/${repo}/pulls?state=closed&sort=updated&direction=desc`);
  ...
  for (const pr of raw) {
    ...
    if (mergedDate < new Date(since))
      continue;
    mergedCandidates.push(pr);
  }
  for (const pr of mergedCandidates) {
    if (prs.length >= MAX_DETAILED_PRS)
      break;
    const detail = githubGet(runtime2, token, `/repos/${owner}/${repo}/pulls/${pr.number}`);
    const reviewsRaw = githubGet(runtime2, token, `/repos/${owner}/${repo}/pulls/${pr.number}/reviews`);
...
function fetchIssues(runtime2, token, owner, repo, since) {
  const raw = githubGetPaginated(runtime2, token, `/repos/${owner}/${repo}/issues?state=all&sort=updated&direction=desc&since=${since}`);
  const issues = [];
...
function groupReviewsByAuthor(prs) {
  const map = new Map;
  for (const pr of prs) {
...
function groupIssuesByAuthor(issues) {
  const map = new Map;
...
function buildCommunityStats(prs, issues) {
  const map = new Map;
  ...
  for (const pr of prs) {
    if (!pr.merged)
      continue;
    const hasDocLabel = pr.labels.some((l) => l.toLowerCase().includes("doc") || l.toLowerCase().includes("documentation"));
    if (hasDocLabel) {
...
var SYSTEM_PROMPT = `You are a code contribution analyzer for open source projects.
...
function formatPRForAnalysis(pr) {
  const lines = [
...
${formatPRForAnalysis(pr)}` }
  ...
  try {
    ...
    const validComplexities = ["trivial", "medium", "high", "critical"];
...
function classifyAllPRs(runtime2, apiKey, apiUrl, model, prs) {
  const results = new Map;
  for (const pr of prs) {
    const result = classifyPRComplexity(runtime2, apiKey, apiUrl, model, pr);
    results.set(pr.number, result);
  }
  return results;
}
var SCORE_SCALE = 1000000n;
var WEIGHTS = {
  PR: 0.45,
  REVIEW: 0.3,
  ISSUE: 0.15,
...
var COMPLEXITY_BASE_POINTS = {
  trivial: 1,
  medium: 1.5,
  high: 2,
  critical: 3
};
var REVIEW_TYPE_WEIGHT = {
  CHANGES_REQUESTED: 1,
  COMMENTED: 0.7,
  APPROVED: 0.5,
  DISMISSED: 0.3
...
var DEFAULT_LABEL_WEIGHT = 0.7;
function weeksBetween(dateStr, now) {
  const d = new Date(dateStr);
  return Math.max(0, (now.getTime() - d.getTime()) / (7 * 24 * 60 * 60 * 1000));
}
function calculatePRScore(pr, complexity, now) {
...
function calculateReviewScore(review) {
  if (review.author === review.prAuthor)
    return 0;
  ...
  const isInternal = review.authorAssociation === "MEMBER" || review.authorAssociation === "COLLABORATOR";
  const authorityFactor = isInternal ? 1 : 0.7;
  return typeWeight * depthBonus * authorityFactor;
}
function calculateIssueScore(issue) {
  const basePoints = 1;
  let resolutionBonus = 1;
...
function calculateCommunityScore(stats) {
  return 0.5 * stats.docPRsMerged + 0.3 * stats.issuesTriaged + 0.2 * stats.firstContributorHelps;
}
function calculateCompositeScore(prScore, reviewScore, issueScore, communityScore) {
  return WEIGHTS.PR * prScore + WEIGHTS.REVIEW * reviewScore + WEIGHTS.ISSUE * issueScore + WEIGHTS.COMMUNITY * communityScore;
}
function scaleForContract(score) {
  return BigInt(Math.round(score * Number(SCORE_SCALE)));
}
function scoreAllContributors(prsByAuthor, reviewsByAuthor, issuesByAuthor, communityByAuthor, contributorMapping, now) {
  ...
  for (const username of allAuthors) {
    const address2 = contributorMapping[username];
    if (!address2)
      continue;
    ...
    const compositeScore = calculateCompositeScore(prScore, reviewScore, issueScore, communityScore);
    results.push({
...
var configSchema = exports_external.object({
  ...
  repoName: exports_external.string(),
...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log(`Scoring ${config.repoOwner}/${config.repoName} for last ${config.cycleDays} days`);
  const prs = fetchMergedPRs(runtime2, githubToken, config.repoOwner, config.repoName, since);
  runtime2.log(`Fetched ${prs.length} merged PRs`);
  const issues = fetchIssues(runtime2, githubToken, config.repoOwner, config.repoName, since);
  runtime2.log(`Fetched ${issues.length} issues`);
  if (prs.length === 0) {
    runtime2.log("No merged PRs found in scoring window, skipping");
    const donRuntime2 = runtime2.usingTheDons();
    const emptyPayload = encodeAbiParameters(parseAbiParameters("bytes32 repoId, address[] contributors, uint256[] scores"), [keccak256(toHex(`${config.repoOwner}/${config.repoName}`)), [], []]);
    donRuntime2.report({
  ...
  const contributorScores = scoreAllContributors(prsByAuthor, reviewsByAuthor, issuesByAuthor, communityStats, config.contributorMapping, now);
  runtime2.log(`Scored ${contributorScores.length} contributors`);
  for (const cs of contributorScores) {
    runtime2.log(`  ${cs.githubUsername}: composite=${cs.compositeScore.toFixed(4)} | PR=${cs.prScore.toFixed(3)} Review=${cs.reviewScore.toFixed(3)} Issue=${cs.issueScore.toFixed(3)} Community=${cs.communityScore.toFixed(3)}`);
  }
  const donRuntime = runtime2.usingTheDons();
  const repoId = keccak256(toHex(`${config.repoOwner}/${config.repoName}`));
  const contributors = contributorScores.map((cs) => cs.address);
  const scores = contributorScores.map((cs) => cs.scaledScore);
  const encodedPayload = encodeAbiParameters(parseAbiParameters("bytes32 repoId, address[] contributors, uint256[] scores"), [repoId, contributors, scores]);
  donRuntime.report({
  ...
  const summary = contributorScores.map((cs) => `${cs.githubUsername}=${cs.compositeScore.toFixed(2)}`).join(", ");
