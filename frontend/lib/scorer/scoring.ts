...
export function calculatePRScore(
	...
	const reviewDepthMultiplier = pr.reviewCount < 1 ? 0.7 : 1 + 0.15 * reviewRounds
...
export function calculateCommunityScore(stats: CommunityStats): number {
	const commitPoints = Math.log2((stats.commits ?? 0) + 1)
	return (
...
function walletFor(mapping: ContributorMapping, username: string): string | undefined {
	if (mapping[username]) return mapping[username]
	const lower = username.toLowerCase()
	if (mapping[lower]) return mapping[lower]
	for (const [login, wallet] of Object.entries(mapping)) {
		if (login.toLowerCase() === lower) return wallet
	}
	return undefined
}
...
export function scoreAllContributors(
	...
	const authors = new Set<string>([
		...Object.keys(contributorMapping),
		...prsByAuthor.keys(),
	...
	for (const username of authors) {
		const address = walletFor(contributorMapping, username)
		if (!address) continue
		const key = username.toLowerCase()
		const seenKey = `${key}:${address.toLowerCase()}`
		if (seen.has(seenKey)) continue
		seen.add(seenKey)

		const prs = prsByAuthor.get(key) ?? prsByAuthor.get(username) ?? []
		const reviews = reviewsByAuthor.get(key) ?? reviewsByAuthor.get(username) ?? []
		const issues = issuesByAuthor.get(key) ?? issuesByAuthor.get(username) ?? []
		const community = communityByAuthor.get(key) ?? communityByAuthor.get(username) ?? {
			docPRsMerged: 0,
			issuesTriaged: 0,
			firstContributorHelps: 0,
			commits: commitCounts.get(key) ?? 0,
		}
		if (!community.commits) community.commits = commitCounts.get(key) ?? 0
